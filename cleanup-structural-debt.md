...
└── src/
    ...
    ├── ui/picker/              # picker feature (+ resolve-image-uri trio)
...
- The `resolve-image-uri` platform split is correct (web URL vs native runtime lookup); the defect is its UI-folder location and its `background` name.
...
- ✅ `f967838` — **W4 components**: `controls/` + `ui/` → `src/components/{picker,tuner,ui,form,theme}`; public subpaths `./ui` → `./picker`, `./controls` → `./tuner`. `resolve-image-uri` rename deferred (native-coupled).
