...
└── src/
    ...
    ├── ui/picker/              # picker feature (+ resolve-thumbnail-uri trio)
...
- **W2 — Drivers. ✅ SHIPPED (`0b25113`).** Moved `src/web/ → web-driver/` at the repo root (peer to `android/`/`ios/`); added a `web-driver/index.ts` barrel (the driver's public surface the runtime consumes) and `web-driver/test/`. Deviation: `clamp` stayed in `web-driver/tuning.ts` rather than moving to `src/lib/` — it's a single-use local; hoisting it to a shared lib pre-emptively is the speculative abstraction the ontology step warns against (do it when a second consumer appears). Repointed `build:shaders`/`check:shaders` output, the three tsconfig scopes + biome + knip, and all `src/web/` doc/native-comment cross-references.
...
- **W4 — Components. ✅ SHIPPED (`f967838`).** Collapsed `src/controls/` + `src/ui/` into `src/components/`: two features (`picker/`, `tuner/`), shared `ui/` primitives, `form/` (lifted to `components/form` since both the primitives and the tuner use it), and `theme/`. Fixed D6's inversion by renaming the public subpaths `./ui` → `./picker` and `./controls` → `./tuner` (demo + nativewind + parity test repointed). DEFERRED: the `resolve-thumbnail-uri` rename off "background" — its `resolveThumbnailUri` is a native Expo Function registered in Kotlin/Swift, so a clean rename needs the native change + a rebuild; folded into the native vocab work, not this JS reorg.
...
- The `resolve-thumbnail-uri` platform split is correct (web URL vs native runtime lookup); the defect is its UI-folder location and its `background` name.
...
- ✅ `55f6d77` — **W1 (preset-book types, confirmed done + one rename)**: the `kaleidoscope.preset-book.types.ts` lexicon is structurally complete and canonical (the runtime contract `effect.types.ts` builds on `KaleidoscopeLayer` rather than duplicating; the types are the imported source of truth across 30+ files). Fixed the one adjacent stale-vocab public name: `BACKGROUND_PRESETS`/`BackgroundPresetName` → `CATALOG_IMAGE_IDS`/`CatalogImageId` (`catalog/images/image-ids.ts`). Also renamed `PresetSource` → `ImageSource` (`catalog/images/image.types.ts`, `3de2754`), de-plating the 49 loader pairs.
- ✅ `78b5d3a` — **W6 lib**: `src/test-id.ts` → `src/lib/test-id.ts` (an id-maker, not a test); 6 consumers repointed.
- ✅ `f967838` — **W4 components**: `controls/` + `ui/` → `src/components/{picker,tuner,ui,form,theme}`; public subpaths `./ui` → `./picker`, `./controls` → `./tuner`. `resolve-thumbnail-uri` rename deferred (native-coupled).
