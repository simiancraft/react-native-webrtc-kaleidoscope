...
//
// Inputs: the repo .frag form (uniforms filled from the sibling <name>.ts
// *_CONTROLS defaults; uTime/uResolution are engine-provided) OR a ShaderToy
// mainImage snippet (rendered as-is). Generates a self-contained HTML file and
// opens it in your browser; re-run after edits to refresh.
//
...
type Side = { label: string; fragSrc: string; uniforms: UniformSpec[]; notes: string[] };
...
// Find a *_CONTROLS-style array in a sibling .ts module and flatten to defaults.
function controlDefaults(mod: Record<string, unknown>): Map<string, number | number[]> {
  ...
  for (const v of Object.values(mod)) {
    if (
      ...
      for (const c of v as Array<{ name: string; default: number | number[] }>) {
...
async function resolveSide(raw: string, path: string, label: string): Promise<Side> {
  ...
  if (isFrag) {
    ...
    const notes: string[] = [];
    let defaults = new Map<string, number | number[]>();
    if (path.endsWith('.frag')) {
      const tsPath = join(dirname(resolve(process.cwd(), path)), `${basename(path, '.frag')}.ts`);
      try {
        defaults = controlDefaults((await import(tsPath)) as Record<string, unknown>);
      } catch {
    ...
    return { label, fragSrc, uniforms, notes };
  }
...
function html(sides: [Side, Side], vert: string): string {
  const data = JSON.stringify({ vert, sides });
  return `<!doctype html>
...
function setUniforms(gl, prog, locs, uniforms, time, W, H) {
...
function makeSide(canvas, side, errPanel) {
  ...
  return {
    ...
    draw(time) {
      ...
      gl.viewport(0, 0, w, h);
      gl.useProgram(prog);
      setUniforms(gl, prog, locs, side.uniforms, time, w, h);
    ...
    measure(time, k) {
      ...
      gl.useProgram(prog);
      setUniforms(gl, prog, locs, side.uniforms, time, m.w, m.h);
