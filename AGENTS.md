...
- **ESLint resolves its parser from `tools/lint`, not from the root, and that is deliberate.** The root compiles with TypeScript 7 (`tsc` IS the Go compiler now; `tsgo` and `@typescript/native-preview` are gone). `typescript-eslint` throws outright on TypeScript >= 7 (`typescript-eslint#10940`), so `tools/lint` is a private workspace whose only job is to pin the TypeScript 6 API next to `@typescript-eslint/parser` and `eslint-plugin-react-compiler`; `eslint.config.mjs` loads both through a `createRequire` rooted at `tools/lint/package.json`, and Node then resolves `typescript` inside that folder. Two consequences: `eslint.config.mjs` MUST stay at the repo root, because ESLint refuses to lint files outside its config's base path and the linted sources are `src/components` and `catalog/**`; and `tools/lint` is in `knip.json`'s `ignoreWorkspaces`, because knip cannot follow a `createRequire` and would report both packages as unused. Delete the workspace when typescript-eslint ships TypeScript 7 support, and move the two deps back to the root.
