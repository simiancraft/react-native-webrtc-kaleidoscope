...
## Dependency upgrades: an Android build is MANDATORY (READ THIS before bumping anything)

**Never ship, merge, or recommend a dependency upgrade that can reach the binary without building and running the Android app.** A green `bun run check`, a green CI run, and a working web demo are NOT evidence: none of them load the app on Android. Web fetches assets as URLs and runs in JS, so it never touches autolinking, Gradle, the manifest merger, native asset bundling, or ABI packaging, and `check:android` is only a Kotlin compile of this module. This is the fastest way to break a React Native app.

Treat as binary-affecting: `react-native`, `expo`, `expo-modules-core`, `@expo/config-plugins`, `react-native-webrtc` (and the LiveKit fork), anything with an `android/` dir or a podspec, anything a config plugin touches, the demo's `overrides`, and any lockfile refresh (it can move transitive native deps underneath a JS-only bump). Only a pure types or tooling bump that provably never reaches the app is exempt.

Required, in order, before calling an upgrade verified:

1. `cd demo && bunx expo-doctor`; resolve what it reports (SDK alignment, peer deps, duplicate native modules). Do not settle version compatibility by reading a changelog.
2. A FRESH prebuild, not a reused one: `cd demo && bunx expo prebuild --platform android --clean --no-install`. A stale `demo/android` bakes in an old config-plugin run and hides exactly the breakage a consumer hits.
3. A standalone build: `cd demo/android && ./gradlew assembleRelease` (signs with the debug keystore; embeds the JS, so no Metro).
4. Install and drive it on an emulator with the Maestro harness (`android-emulator-harness`; `android-emulator-mask-testing` for segmentation), and gate on logcat: no `FATAL EXCEPTION`, `ANR`, or `UnsatisfiedLinkError`. MediaPipe tasks-vision ships no x86_64 JNI library, so segmentation only runs on a 32-bit `x86` image; on x86_64 the `libmediapipe_tasks_vision_jni.so not found` error is the emulator ABI, not a regression.
5. Hand the maintainer a device pass for anything the emulator cannot judge (frame rate, real camera, segmentation quality).

When an upgrade breaks Android and the cause is unclear, walk the dependency history back to the last version that builds and runs, ship that, then walk forward one change at a time with this same check at every step. Lifeguides (`~/Simiancraft_Programming/Lifeguides/lifeguides-application`, Expo SDK 54 / RN 0.81) is a known-good consumer recipe to compare against.
...
5. **The composite is background-source-agnostic; that is the extensibility model.** Compositing is per-layer: a layer renders its content into a scratch, then `composite-subject` / `composite-masked` stencil it to the person through the mask, or the over/additive blend lays a background layer down. The mask stencil does not care what a layer's texture is. Layers differ ONLY in how that texture is produced: an `image` layer, the blurred camera (`blur`), or a generative GLSL shader (`plasma`, `clouds`, and the rest of the catalog). A new shader dropped into `catalog/shaders/<name>/` (single source → codegen to all platforms) gets the canonical upright frame and composites through the mask with ZERO orientation work; only its raw compute cost varies by device, a function of the shader and the device, not of orientation (the segmentation tier `targetShortSide` trades mask resolution, a separate knob). The flip/rotate transform ops are debug/utility table-stakes, not the product surface; the product is the masked-background composite.
...
## Assets reach native two different ways: do not conflate them (READ THIS before adding any image, shader, or thumbnail)

Web fetches every asset as a JS URL, so a miswired asset RENDERS FINE ON WEB and is blank or wrong on a native standalone build. A dev-client + Metro build hides it too (Metro serves the asset over its dev server). Only a STANDALONE build (EAS `preview`, or a release APK/IPA) exercises the real native path. **Verify any asset change on a standalone build, never web or a Metro dev-client alone.** This has bitten twice (the demo thumbnails; the `clouds` composite), both looking correct on web.

1. **The literal texture the compositor samples — an `image` layer's `source`, or a generative shader — lives IN THE NATIVE BINARY and is resolved BY ID, not by the JS value.** The prebuild plugin (`plugin/src/lib/preset-book.ts` → `collectReferencedAssets`) parses the book as TEXT and copies each referenced WebP to `images/<id>.webp` in the app target; the native GL/Metal compositor reads it by id. The JS `source` value is WEB-ONLY (a URL). For it to bundle: an `image` layer's `id` MUST equal the asset basename, and its `source` must be a statically-parseable `require('./x.webp')` literal, a single named import from `.../images/<cat>/<leaf>`, or a `const X = ...require('./x.webp')...` binding. Anything the parser cannot follow is skipped (it warns at prebuild, "skipping its asset"); the layer then renders on web and is blank on device. Generative shaders are the analogue: codegen'd into the native sources by `build:shaders`, never shipped as a runtime JS string.

2. **The picker `thumbnail` is a UI `<Image>` — a DIFFERENT path from the compositor.** In a consumer's single-file `kaleidoscope.preset-book.ts`, a thumbnail must be a bundled-asset reference: a bare `require('./thumb.webp')` (Metro embeds it; the RN asset registry loads the number via `resolveImageUri`'s number-passthrough) or an imported image module. NEVER `Asset.fromModule(require('./thumb.webp')).uri` in a single-file book: `.uri` is a web-shaped URL that does not resolve to the embedded asset in a native standalone build (that was the bug). The library's packaged composites instead platform-split, because the prebuild's `collectCompositeThumbRefs` bundles `<name>.thumb.webp` ONLY for IMPORTED composites, never an inline `thumbnail:` require. So a NEW packaged composite needs all three: `<name>.ts` with `thumbnail: '<name>-thumb'` (native string id), `<name>.web.ts` with `thumbnail: Asset.fromModule(require('./<name>.thumb.webp')).uri` (web), and the `browser` export condition in `package.json`. Missing any one renders the tile blank (that was the `clouds` bug).

3. **When extending, check:** (a) the new asset renders on a STANDALONE native build, not just web/dev-client; (b) an image layer's `id == basename` and its `source` is statically parseable (watch the prebuild log for "skipping its asset"); (c) a packaged composite has all three pieces above; (d) a consumer thumbnail is a bare `require()`, not wrapped in `Asset.fromModule().uri`.
...
bun run typecheck:test       # tsc --noEmit on src + test against tsconfig.test.json
bun run typecheck:demo       # tsc --noEmit on demo/
bun run build                # emit dist/ (tsc -p tsconfig.build.json) then copy:assets
...
bun run refresh:android      # npm install in demo/ (the demo is npm-managed; never bun install there, see gotcha below)
...
- **`app.plugin.js` is a one-line CJS shim over a committed `plugin/build`.** The package is `"type": "commonjs"`; `app.plugin.js` is `module.exports = require('./plugin/build')`. The real implementation is TypeScript in `plugin/src/` (`{android,ios,lib}`), compiled by `bun run build:plugin` (tsc + a `@generated` banner + biome) into `plugin/build/`, which is **committed to git** and guarded by the `check:plugin` drift gate. Why committed and not `dist/`: Expo loads `app.plugin.js` with `require()` from its REAL path, which on EAS is the repo root (the demo consumes the lib via `file:..`) where neither `node_modules` NOR the gitignored `dist/` is present; a committed `plugin/build` is the only output guaranteed there. For the same reason the compiled output touches nothing but Node builtins at load: the mods register by mutating `config.mods.<platform>` directly (not `withDangerousMod`), and `@expo/config-plugins` is `require.resolve`'d lazily from the CONSUMER at mod-execution time. Edit `plugin/src/`, then `bun run build:plugin`; never hand-edit `plugin/build/`, and never point the shim at `dist/`.
- **ESLint resolves its parser from `tools/lint`, not from the root, and that is deliberate.** The root compiles with TypeScript 7 (`tsc` IS the Go compiler now; `tsgo` and `@typescript/native-preview` are gone). `typescript-eslint` throws outright on TypeScript >= 7 (`typescript-eslint#10940`), so `tools/lint` is a private, standalone package (its own `package.json` and `bun.lock`; deliberately NOT a workspace) whose only job is to pin the TypeScript 6 API next to `@typescript-eslint/parser` and `eslint-plugin-react-compiler`; `eslint.config.mjs` loads both through a `createRequire` rooted at `tools/lint/package.json`, and Node then resolves `typescript` inside that folder. Three consequences: `eslint.config.mjs` MUST stay at the repo root, because ESLint refuses to lint files outside its config's base path and the linted sources are `src/components` and `catalog/**`; `check:react-compiler` installs `tools/lint` itself (`bun install --cwd tools/lint --frozen-lockfile`, a no-op once installed), so CI and the pre-commit hook need no extra step; and the root `package.json` must NEVER gain a `workspaces` field for it. That field ships in the published manifest, and the one release cut with it (2.7.11) failed at `npm publish` with `ENEEDAUTH` under trusted publishing and never reached npm. Delete the workspace when typescript-eslint ships TypeScript 7 support, and move the two deps back to the root.
...
- **Demo Metro picks up `src/` directly through the `file:..` symlink.** The demo's `package.json` declares the library as `file:..`, which npm installs as a symlink (`demo/node_modules/react-native-webrtc-kaleidoscope -> ../..`), and Metro reads the package's `react-native` exports condition pointing at `src/index.ts`. Changes to `src/` take effect without a publish, but the demo's `bun run start` does NOT run `bun run build` first by default; the `demo*` scripts at the repo root prefix `bun run build &&` so they do.
...
- **The demo is npm-managed, and the library MUST be a symlink inside it. Never run `bun install` in `demo/`.** `demo/package.json` declares the library as `file:..` and `demo/package-lock.json` records it as `"link": true`, so `npm install` symlinks the repo root into `demo/node_modules`. A package manager that COPIES a `file:` dependency instead of linking it (bun does) copies `..` into its own subtree, which recurses (`demo/node_modules/react-native-webrtc-kaleidoscope/demo/node_modules/...`) until the path exceeds the filesystem limit. That has happened (the command that did it was never traced; a `bun install` in `demo/` is the likely cause): a 79-level copy broke `check:kotlin` (ktlint: `File name too long`) and made every EAS build fail at upload with `ENAMETOOLONG`, while local Gradle builds kept passing, so it went unnoticed for weeks. The demo moved from bun to npm in `2190ad1` precisely because of bun's `file:..` handling. Diagnose with `stat -c %F demo/node_modules/react-native-webrtc-kaleidoscope` (must print `symbolic link`); repair with `rm -rf` on that one entry, then `npm install` in `demo/`.
