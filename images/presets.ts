// Catalog of the standalone-background plates the library ships. Plates live
// under images/<category>/, filed by their taxonomy category, several leaves per
// folder; each name here is a leaf, which is the plate id an `image` layer uses
// as its `source` on native (the bundled WebP basename, globally unique) and the
// key the WebP URL resolves to on web. The cutout plates that only feed packaged
// composites (fairy-caves, spaceship, wizard-tower) are not listed here.
// Consumed by:
...
//   - the per-plate loader pairs (images/<category>/<leaf>.ts and .web.ts), which
...
//
// To add a plate: append the leaf here, create images/<category>/ with the
// optimized <leaf>.webp plus the <leaf>.ts / <leaf>.web.ts loader pair, and add
// the ./images/<category>/<leaf> and .webp package exports. No native change is
