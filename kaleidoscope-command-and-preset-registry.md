...
- ✅ **Precompiler, Android copy** (Commit 11): `app.plugin.js` parses `kaleidoscope.presets.ts` at `expo prebuild` and copies only the referenced backgrounds into the Android bundle. Handles both library presets (imported identifier -> `.webp` subpath) and consumer assets (`require('./asset')` literal). Verified on a real `expo prebuild --platform android`.
- ✅ **Latent move / tree-shaking** (Commit 10): removed the committed background webps from `android/src/main/assets` and the iOS resource bundle; `src/backgrounds` stays canonical (web exports + prebuild source). The library no longer auto-bundles backgrounds; the prebuild copy is the only native path. Android loads the copied asset by name through the unchanged factory; iOS bundle-search rides with the native pass.
- ✅ **Book is single source of truth** + **consumer-add image** (Commit 15): the demo derives all banks from the book by shader, and a demo-owned `studio-loft.webp` (in `demo/assets`, not the package) is referenced via a relative require — it shows in the demo automatically and the prebuild copies it (12 total). Commenting a preset out of the book drops it from both the demo and the prebuild.

- ✅ **Three-verb surface** (Commits 13/14 superseded): `bindKaleidoscope(track, { presets, onTrack })` returns `{ kaleidoscope, transform, mask }`. `kaleidoscope` is the art axis (typed, narrows opts to the preset's shader, `null` clears); `transform({ flip, rotate })` is absolute geometry, rotate snapped to 90° and decomposed to the existing discrete ops; `mask({ hardness, threshold })` writes the segmentation edge (defaults 0.5/0.5). Public tuning setters removed; transforms left the book. Demo rebuilt to Kaleidoscope / Transform / Mask + Reset. Verified on web.
- ✅ **README** (Commit 17): leads with the three verbs (chromonym style); Use rewritten around the preset book + `bindKaleidoscope`.

Remaining (the native pass): **native plasma** (Android/iOS processor + generalizing the parameter channel to arbitrary uniforms, e.g. `setShaderUniforms`); **native three-phase reorder**; **native registration** (directory discovery so native loads copied-by-id assets incl. consumer images; iOS prebuild copy + bundle-search). Smaller follow-ups: align the `/livekit` adapter to the verb/book surface; re-normalize blur sigma + plasma speed/scale to 0..1; codegen shader contracts (#30).
