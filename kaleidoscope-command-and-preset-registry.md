...
Remaining: **Commit 14** (remove the public tuning setters; the debug panel still uses them); **native plasma** (Android/iOS processor + generalizing the parameter channel to arbitrary uniforms, e.g. `setShaderUniforms`); **native three-phase reorder** (Phase B 3/4); **native registration** (directory discovery so native loads copied-by-id assets, incl. consumer images; iOS prebuild copy + bundle-search) — the native pass; README/migration (Commit 17).
